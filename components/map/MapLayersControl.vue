              ...
              <div v-for="layer in group.layers" :key="layer.id" class="flex gap-3 py-3">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-start justify-between gap-2">
                    <p class="flex min-w-0 items-center gap-2 text-sm font-semibold leading-snug"><span v-if="layer.source" class="size-2 shrink-0 rounded-full" :style="{ backgroundColor: layer.source.color }"/>{{ layer.label }}</p>
                    <span v-if="layer.status === 'soon'" class="ui-label shrink-0 rounded-full bg-ink/5 px-2 py-1 text-[8px] text-ink/42">Próx.</span>
                    <span v-else-if="layer.source" class="shrink-0 font-mono text-[9px] text-ink/38">{{ formatCount(layer.source.featureCount) }}</span>
                    <span v-else class="ui-label shrink-0 rounded-full bg-river/8 px-2 py-1 text-[8px] text-river">Base</span>
                  </div>
