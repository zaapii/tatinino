...
const visibleLayers = computed(() => props.layers.filter(layer => layer.enabled && layer.source))
</script>
  ...
  <section v-if="visibleLayers.length" class="surface-panel absolute bottom-4 left-4 z-20 hidden w-[252px] rounded-xl p-3.5 sm:block">
    ...
    <ul class="mt-3 space-y-2">
      <li v-for="layer in visibleLayers.slice(0, 5)" :key="layer.id" class="flex items-center justify-between gap-3 text-[10px] text-ink/62">
        <span class="flex min-w-0 items-center gap-2"><span class="h-0.5 w-5 shrink-0 rounded-full" :style="{ backgroundColor: layer.source?.color }"/><span class="truncate">{{ layer.label }}</span></span>
        <span class="font-mono text-[8px] text-ink/32">CAD</span>
      </li>
    </ul>
